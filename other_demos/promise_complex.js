...
function promiseResolutionHandler(promise2, x, resolve, reject) {
    ...
    if (x instanceof MyPromise) {
        if (x.status === STATUS.PENDING) {
            // x 还在处理态
            x.then((xValue) => {
                promiseResolutionHandler(promise2, xValue, resolve, reject);
            }, reject);
        } else {
            // x 处理结束, 将处理结果传给当前对象
...
class MyPromise {
    ...
    then(onFulfilled, onRejected) {
        ...
        //                     let value = onFulfilled(innerValue);
        //                     // 记录回调的执行结果, 并改变当前promise对象(新的Promise对象)的状态和值
        //                     resolve(value);
        //                 } catch (err) {
        //                     reject(err);
        //                 }
        ...
        //         });
        //         // 1.2 暂存 onRejected
        //         rejectQueue.push((innerReason) => {
        //             setTimeout(() => {
        //                 try {
        //                     let reason = onRejected(innerReason);
        //                     resolve(reason);
        //                 } catch (err) {
        //                     reject(err);
        //                 }
        ...
        //     promise2 = new MyPromise((resolve, reject) => {
        //         try {
        //             // 根据Promise对象的状态决定执行哪个回调函数
        //             let value = this.status === STATUS.FULFILLED ? onFulfilled(this.value) : onRejected(this.value);
        //             resolve(value); //改变promise对象的状态和值
        //         } catch (err) {
        //             reject(err);
        //         }
        //     });
